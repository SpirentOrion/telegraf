...
func (c *CloudStressAgent) Start(acc telegraf.Accumulator) error {
	...
	log.Printf("Started the cloudstress agent consumer service. Subscribing on *:%d\n", c.SubscriberPort)
...
func (c *CloudStressAgent) Stop() {
	...
	close(c.done)
	log.Printf("Stopping the cloudstress agent consumer service\n")
	if err := c.subscriber.Close(); err != nil {
		log.Printf("Error closing cloudstress agent consumer service: %s\n", err.Error())
	}
}
...
func (c *CloudStressAgent) Gather(acc telegraf.Accumulator) error {
	...
	fmt.Printf("Processed %f cloud-stress agent metrics per second\n", rate)
	return nil
}
	...
	for {
		...
		case msg := <-c.msgs:
			...
			case result.Update_CPU:
				...
				c.acc.AddFields(loadType, metrics, tags, time.Now())
				c.currValue++
